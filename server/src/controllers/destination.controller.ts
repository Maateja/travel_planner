...
export const discoverDestinations = async (req: Request, res: Response) => {
    try {
            ...
            try {
                ...
                if (jsonMatch) {
                    ...
                    aiResults = aiResults.filter(p => {
                        ...
                        if (!isClose) console.log(`[Discovery] AI result "${p.name}" filtered out: ${dist.toFixed(1)}km > ${r}km`);
                        return isClose;
                    });
                    ...
                    for (const p of aiResults) {
                        try {
                            ...
                            if (!exists) await new Destination(p).save();
                        } catch(e) {}
                    }
                }
            } catch (aiErr: any) {
        ...
        if (msg.includes("429")) msg = "The AI is currently busy with many requests. Please wait a few seconds and click 'Try Again'.";
        if (msg.includes("Quota")) msg = "AI daily limit reached or model busy. Please try again in 1 minute.";
        
        res.status(500).json({ message: "AI Discovery Error: " + msg });
    }
};
export const discoverCityDetails = async (req: Request, res: Response) => {
    try {
