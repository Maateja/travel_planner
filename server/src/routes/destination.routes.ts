...
router.get('/', async (req, res) => {
    try {
        ...
        if (budget_max) query.estimated_budget_max = { $lte: Number(budget_max) };
        if (days) query.recommended_days = { $lte: Number(days) };
...
router.get('/random', async (req, res) => {
    try {
        ...
        if (budget_max) match.estimated_budget_max = { $lte: Number(budget_max) };
        if (days) match.recommended_days = { $lte: Number(days) };
